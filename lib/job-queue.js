...
async function listJobsByStatus(statuses = [], olderThanMs = 0, limit = 100) {
  const wanted = new Set(Array.isArray(statuses) ? statuses : []);
  const cutoff = Date.now() - Math.max(Number(olderThanMs) || 0, 0);
  const max = Math.min(Math.max(Number(limit) || 100, 1), 1000);
  const out = [];
  const add = job => {
    if (!job || (wanted.size && !wanted.has(job.status))) return;
    const updated = Date.parse(job.updatedAt || job.finishedAt || job.createdAt || 0);
    if (cutoff && (!updated || updated > cutoff)) return;
    out.push(job);
  };
  if (!REDIS_URL) {
  ...
  const redis = await getRedis();
  for await (const key of redis.scanIterator({ MATCH: JOB_PREFIX + "*", COUNT: 100 })) {
...
module.exports = {
  ...
  recoverExpiredJobs, promoteRetries, listJobsByStatus, getQueueInfo, getQueueHealth
};
