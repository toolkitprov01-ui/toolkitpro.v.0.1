...
"time-duration-calculator":v=>{const [a,b]=v.split(/[,\n]+/).map(x=>x.trim());const parse=x=>{const m=x.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?$/);if(!m)throw new Error("Use HH:MM or HH:MM:SS");return Number(m[1])*3600000+Number(m[2])*60000+Number(m[3]||0)*1000};let ms=parse(b)-parse(a);if(ms<0)ms+=86400000;return `${Math.floor(ms/3600000)} hours ${Math.floor(ms/60000)%60} minutes ${Math.floor(ms/1000)%60} seconds`},
"timezone-converter":v=>{const parts=v.trim().split(/\s+/);const zone=parts.pop()||"UTC";const iso=parts.join(" ");const d=new Date(iso);if(Number.isNaN(d.getTime()))throw new Error("Use ISO datetime followed by IANA timezone, e.g. 2026-09-18T12:00:00Z Asia/Dhaka");return new Intl.DateTimeFormat(undefined,{dateStyle:"medium",timeStyle:"long",timeZone:zone}).format(d)},
...
"csv-to-json":v=>{const parseLine=line=>{const out=[];let cur="",quoted=false;for(let i=0;i<line.length;i++){const ch=line[i];if(ch==='"'&&line[i+1]==='"'){cur+='"';i++;continue}if(ch==='"'){quoted=!quoted;continue}if(ch===','&&!quoted){out.push(cur);cur="";continue}cur+=ch}out.push(cur);return out};const [head,...rows]=v.trim().split(/\r?\n/);const keys=parseLine(head||"");return JSON.stringify(rows.filter(r=>r.trim()).map(r=>{const vals=parseLine(r);return Object.fromEntries(keys.map((k,i)=>[k,vals[i]??""]))}),null,2)},
...
"calorie-calculator":v=>{const [age,weight,height,activity=1.2]=v.split(/[,\s]+/).map(Number);if(!(age>0&&weight>0&&height>0))throw new Error("Use age, weight (kg), height (cm), activity factor");const bmr=10*weight+6.25*height-5*age+5;return String(Math.round(bmr*(Number(activity)||1.2)))+" kcal/day (approx.)"},
"bmr-calculator":v=>{const [age,weight,height,sex="m"]=v.split(/[,\s]+/);if(!(Number(age)>0&&Number(weight)>0&&Number(height)>0))throw new Error("Use age, weight (kg), height (cm), sex");const b=10*Number(weight)+6.25*Number(height)-5*Number(age)+(String(sex).toLowerCase().startsWith("f")?-161:5);return String(Math.round(b))+" kcal/day (approx.)"},
"body-fat-calculator":v=>{const [waist,neck,height]=v.split(/[,\s]+/).map(Number);if(!(waist>neck&&neck>0&&height>0))throw new Error("Use waist, neck and height in cm; waist must be greater than neck");const pct=(495/(1.0324-0.19077*Math.log10(waist-neck)+0.15456*Math.log10(height)))-450;return pct.toFixed(2)+"% (approx.)"},
"text-to-uppercase":v=>v.toUpperCase(),
"text-to-lowercase":v=>v.toLowerCase(),
"title-case-converter":v=>v.toLowerCase().replace(/(^|[^\\p{L}\\p{N}])([\\p{L}])/gu,(m,p,c)=>p+c.toUpperCase()),
...
"base64-encoder":v=>btoa(String.fromCharCode(...new TextEncoder().encode(v))),
"base64-decoder":v=>new TextDecoder().decode(Uint8Array.from(atob(v),c=>c.charCodeAt(0))),
...
"json-stringify":v=>JSON.stringify(JSON.parse(v),null,2),
...
export function render({tool}){
  if(tool.category==="image"){
    return `<div class="tool-form"><div class="tool-toolbar"><span class="tool-toolbar-title">${tool.bn||tool.name}</span></div><input id="imageFile" type="file" accept="image/*">${tool.id==="image-resizer"?`<div class="tool-actions" style="margin-top:10px"><input id="imageWidth" type="number" min="1" max="10000" placeholder="Width (px)" aria-label="Width in pixels"><input id="imageHeight" type="number" min="1" max="10000" placeholder="Height (px)" aria-label="Height in pixels"></div>`:""}${tool.id==="image-cropper"?`<label style="display:block;margin-top:10px">Crop size (px) <input id="cropSize" type="number" min="1" max="10000" placeholder="Square size"></label>`:""}<div class="tool-actions" style="margin-top:10px"><button class="tool-action" id="imageRun" type="button">প্রসেস করুন</button><a class="tool-action" id="imageDownload" hidden download>ডাউনলোড</a></div><label id="imageQualityWrap" style="display:block;margin-top:10px">Quality <input id="imageQuality" type="range" min="0.1" max="1" step="0.05" value="0.8"></label><div id="imagePreview" style="margin-top:12px"></div><p class="tool-note">Image processing আপনার ব্রাউজারেই হয়; ফাইল সার্ভারে আপলোড করা হয় না।</p></div>`;
  }
...
export function mount({tool,root}){
  if(tool.category==="image"){
    ...
    const run=()=>{const f=file.files?.[0];if(!f)return;const img=new Image();img.onload=()=>{const c=document.createElement("canvas");let w=img.naturalWidth,h=img.naturalHeight;
      if(tool.id==="image-resizer"){const rw=Number(root.querySelector("#imageWidth")?.value),rh=Number(root.querySelector("#imageHeight")?.value);if(rw>0&&rh>0){w=Math.min(10000,rw);h=Math.min(10000,rh)}else if(rw>0){w=Math.min(10000,rw);h=Math.round(img.naturalHeight*(w/img.naturalWidth))}else if(rh>0){h=Math.min(10000,rh);w=Math.round(img.naturalWidth*(h/img.naturalHeight))}else{w=Math.min(2400,w);h=Math.round(img.naturalHeight*(w/img.naturalWidth))}}
      if(tool.id==="image-cropper"){const requested=Number(root.querySelector("#cropSize")?.value);const s=Math.min(w,h,requested>0?requested:Math.min(w,h));const sx=(w-s)/2,sy=(h-s)/2;c.width=c.height=s;c.getContext("2d").drawImage(img,sx,sy,s,s,0,0,s,s)}
