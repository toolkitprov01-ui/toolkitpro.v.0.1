...
app.get("/tool/:id",(req,res)=>{
  ...
  const page=toolPageTemplate.replaceAll("__TOOL_TITLE__",escHtml(tool.bn||tool.name)).replaceAll("__TOOL_DESCRIPTION__",escHtml(tool.description)).replaceAll("__TOOL_URL__",escHtml(url)).replace("__TOOL_JSONLD__",escJson(jsonLd)).replace("__TOOL_ID__",escJson(tool.id));
  cacheSet(key,page,300_000); res.set("Cache-Control","public, max-age=300").send(page);
});
