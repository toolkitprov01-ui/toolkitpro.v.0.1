...
app.get("/api/search", (req, res) => {
  ...
  const results = tools.map(tool => {
    ...
    const haystack = [
      tool.id, tool.name, tool.bn, tool.description, tool.category,
      tool.categoryBn, ...tool.tags, ...tool.synonyms
    ].join(" ").toLowerCase();
...
app.get("/api/health", (req, res) => {
  ...
  res.json({
    ...
    cache: { type: "memory", entries: cache.size, ttlMs: CACHE_TTL_MS },
...
app.get("/api/tools", (req, res) => {
  ...
  const tools = cached || cacheSet("tools", getAllTools(), 300_000);
  res.set("Cache-Control", "public, max-age=60, stale-while-revalidate=300");
...
app.get("/api/tools/:id", (req, res) => {
  ...
  const cached = cacheGet(key);
  const tool = cached || cacheSet(key, getToolById(req.params.id), 300_000);
  res.set("Cache-Control", "public, max-age=60, stale-while-revalidate=300");
...
app.get("/tool/:id", (req, res) => {
  ...
  const cached = cacheGet(key);
  if (cached) {
    res.set("Cache-Control", "public, max-age=300").send(cached);
    return;
  }
